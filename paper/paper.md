...
Finally, `QhX` enhances traditional analysis by generating numerical and interactive visual catalogs that rank periodicity candidates by reliability. These interactive catalogs enable detailed inspection of signal consistency, offering greater interpretability than traditional static plots.
...
3. **Data Management**:
   - `QhX` takes as input time-series data in a simple tabular format containing time, flux (or magnitude), and associated uncertainties. Examples in the documentation illustrate how to map data from other commonly used formats into this structure.
